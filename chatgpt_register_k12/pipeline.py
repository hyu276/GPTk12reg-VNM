...
from chatgpt_register_k12.register.registrar import project_registration_password, register_worker
...
WORKSPACE_PLAN_TYPES = {
    ...
    "free_workspace", "k12", "quorum", "sci", "self_serve_business_usage_based", "team",
}
...
TOKEN_INVALID_MARKERS = (
    "token auth failed", "token authentication failed", "check api token auth failed",
    "token_revoked", "token_invalidated", "invalidated oauth token",
    "authentication token has been invalidated", "token has been invalidated",
...
def create_run_output_dir(config: dict[str, Any], count: int | None = None) -> Path:
    ...
    if not isinstance(output_cfg, dict):
        output_cfg = {}
    runs_dir = Path(str(output_cfg.get("runs_dir") or "runs").strip() or "runs")
    if not runs_dir.is_absolute():
        runs_dir = config_dir / runs_dir
    planned_count = _positive_int(
        count if count is not None else config.get("registration", {}).get("total"), 1
    )
...
def _mail_config_with_proxy(config: dict[str, Any]) -> dict[str, Any]:
    ...
    mail_cfg["_config_dir"] = config.get("_config_dir", ".")
    return mail_cfg


def _mail_config_for_job(config: dict[str, Any], cancel_event: threading.Event | None = None) -> dict[str, Any]:
...
def workspace_state_entries(config: dict[str, Any], workspace_id: str = "") -> list[dict[str, Any]]:
    return list_workspace_entries(workspace_state_path(config), workspace_id or _primary_workspace_id(config))


def preview_mailbox_candidates(config: dict[str, Any], count: int | None = None, workspace_id: str = "") -> list[dict[str, Any]]:
    ...
    for item in configured_mailboxes(_mail_config_with_proxy(config)):
        ...
        available = not target_workspace or workspace_email_available(state_file, target_workspace, address)
        candidates.append({
        ...
        if available:
            available_count += 1
        if count is not None and available_count >= count:
            break
    return candidates


def _resolve_export_output_path(config: dict[str, Any], output_file: str | Path | None = None) -> Path:
    if output_file:
...
def _should_fallback_to_project_password(error: Exception) -> bool:
    ...
    return any(marker in detail for marker in (
        "password", "unsupported login continuation page: error", "auth error page", "otp-only login",
    ))


def _existing_login_with_project_password(
    *, email: str, mail_config: dict[str, Any], proxy: str,
    flaresolverr_url: str, workspace_id: str = "",
) -> dict[str, Any]:
...
def _extract_account_contexts(data: Any) -> list[dict[str, Any]]:
    ...
    def add(raw: Any, is_default: bool = False) -> None:
        ...
        account_id = str(
            account.get("account_id") or wrapper.get("account_id")
            or account.get("id") or wrapper.get("id") or ""
        ).strip()
        role = str(
            account.get("account_user_role") or wrapper.get("account_user_role")
            or wrapper.get("role") or ""
        ).strip()
    ...
    if isinstance(accounts, dict) and isinstance(accounts.get("default"), dict):
        add(accounts["default"], is_default=True)
...
def _fetch_account_context(session, access_token: str, workspace_id: str = "") -> dict[str, str]:
    resp = session.get(
    ...
    if resp.status_code != 200:
        ...
        if resp.status_code in (401, 403):
            raise RuntimeError(f"check API token auth failed: HTTP {resp.status_code}{suffix}")
    ...
    try:
        ...
        detail = _response_excerpt(resp)
        suffix = f", body={detail}" if detail else ""
        raise RuntimeError(f"check API returned non-JSON: HTTP {resp.status_code}{suffix}") from error
    candidates = _extract_account_contexts(data)
    target_workspace = str(workspace_id or "").strip()
    selected = None
    if target_workspace:
        selected = next((item for item in candidates if target_workspace in item.get("_ids", set())), None)
    if selected is None:
        selected = next((item for item in candidates if item.get("_default")), None)
    if selected is None and candidates:
        selected = candidates[0]
    if selected is None:
        return {"plan_type": "", "chatgpt_account_id": "", "account_user_role": ""}
    return {
...
def _apply_account_context(account: dict[str, Any], context: dict[str, str], *, prefix: str = "", overwrite_default: bool = False) -> None:
...
def _active_workspace_id(account: dict[str, Any], workspace_ids: list[str]) -> str:
    ...
    if isinstance(join_results, list):
        for result in join_results:
            ...
            if ws_id and result.get("ok") and result.get("membership_active", True) and (not workspace_ids or ws_id in workspace_ids):
                return ws_id
    return workspace_ids[0] if workspace_ids else ""


def _apply_workspace_export_context(account: dict[str, Any], workspace_id: str, plan_type: str = DEFAULT_WORKSPACE_EXPORT_PLAN) -> None:
    if not workspace_id:
        return
...
def _mark_join_verified_by_check(account: dict[str, Any], workspace_id: str) -> None:
    if not workspace_id:
        return
    ...
    if isinstance(results, list):
        for result in results:
            if isinstance(result, dict) and str(result.get("workspace_id") or "").strip() == workspace_id:
                result["ok"] = True
...
def _has_auth_invalid_join_result(account: dict[str, Any]) -> bool:
    ...
    for result in join_results:
        if isinstance(result, dict):
            detail = " ".join(str(result.get(key) or "") for key in ("error", "body", "membership_detail"))
            if _token_invalidated_detail(detail):
                return True
    return False


def _mark_refresh_status(account: dict[str, Any], status: str, detail: str = "") -> None:
...
def _check_export_token_health(session, account: dict[str, Any], sub2api_cfg: dict[str, Any]) -> tuple[bool, str]:
    access_token = str(account.get("access_token") or "").strip()
    if not access_token:
        return False, "missing access_token"
    endpoint_name = str(sub2api_cfg.get("health_check_endpoint") or DEFAULT_HEALTH_CHECK_ENDPOINT).strip()
    ...
    if endpoint_name.lower() == "check":
        ...
        if _is_workspace_plan(account.get("plan_type")) and not _is_workspace_plan(context.get("plan_type")):
...
def _apply_refreshed_token_payload(account: dict[str, Any], data: dict[str, Any]) -> bool:
...
def _refresh_oauth_tokens_for_account(session, account: dict[str, Any]) -> tuple[bool, str]:
...
def _recover_export_token(
    config: dict[str, Any], session, original: dict[str, Any], export: dict[str, Any], last_error: str,
) -> tuple[bool, str]:
    if not _token_invalidated_detail(last_error) or not hasattr(session, "post"):
    ...
    workspace_id = (
        ...
        or (str(original.get("chatgpt_account_id") or "").strip() if _is_workspace_plan(original.get("plan_type")) else "")
    ...
    try:
        ...
        try:
            ...
            return False, (
                f"{detail}; otp relogin failed: {error}; project password relogin failed: {password_error}"
            )
    for key in ("access_token", "refresh_token", "id_token"):
        if tokens.get(key):
...
def _filter_healthy_export_accounts(
    config: dict[str, Any], account_pairs: list[tuple[dict[str, Any], dict[str, Any]]],
    ...
    try:
        ...
        for original, export in account_pairs:
            ...
            for attempt in range(1, retries + 1):
                try:
                    ok, last_error = _check_export_token_health(session, export, sub2api_cfg)
                except Exception as error:
                    ok = False
            ...
            if not ok and _token_invalidated_detail(last_error):
                recovered, recovery_detail = _recover_export_token(config, session, original, export, last_error)
                if recovered:
                    try:
                        ok, last_error = _check_export_token_health(session, export, sub2api_cfg)
                    except Exception as error:
        ...
        if session:
            session.close()
    logger.info(f"Export health check passed: {len(healthy)}/{len(account_pairs)} accounts")
    return healthy
...
def _verify_workspace_membership(session, access_token: str, workspace_id: str) -> tuple[bool, str]:
    resp = session.get(
...
def run_register(
    config: dict[str, Any], accounts_file: Path, count: int | None = None,
    cancel_event: threading.Event | None = None,
) -> list[dict[str, Any]]:
    reg_cfg = config.get("registration", {})
    proxy_cfg = config.get("proxy", {})
    total = _positive_int(count, 10) if count is not None else _positive_int(reg_cfg.get("total", 10), 10)
        ...
        while pending:
            ...
            for future in done:
                ...
                if result["ok"]:
                    ...
                    logger.info(f"[{result['index']}/{total}] ✓ {account['email']} ({result.get('cost_seconds', 0):.1f}s)")
                else:
                    fail_count += 1
                    logger.warning(f"[{result['index']}/{total}] ✗ {result.get('error', 'unknown')}")
    logger.info(f"Registration complete: {success_count} success, {fail_count} failed")
    return results
...
def run_login_existing(
    ...
    state_file = workspace_state_path(config)
    total = _positive_int(count, 10) if count is not None else _positive_int(config.get("registration", {}).get("total", 10), 10)
    threads = _bounded_workers(_parallel_threads(config, "login_threads", 1), total)
    candidates = [
        item for item in configured_mailboxes(mail_cfg)
        if not workspace_id or workspace_email_available(state_file, workspace_id, item.get("address", ""))
    ][:total]
    logger.info(f"Logging in {len(candidates)} existing account(s) with email OTP, {threads} worker(s)")
    existing = load_accounts(accounts_file)
    results: list[dict[str, Any]] = []
    lock = threading.Lock()
    ...
    def _login_one(candidate: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if workspace_id and not claim_workspace_email(
            state_file, workspace_id, email, mode="login_existing",
            extra={
        ...
        try:
            try:
                ...
                if not _should_fallback_to_project_password(otp_error):
                    raise
                logger.warning(f"[{email}] OTP login unavailable: {otp_error}; trying project fixed password")
                tokens = _existing_login_with_project_password(
                    email=email,
                    mail_config=mail_cfg,
                    proxy=proxy,
            ...
            if workspace_id:
                set_workspace_email_state(
                    state_file, workspace_id, email, "logged_in", mode="login_existing",
                    extra={
                        "provider": candidate.get("provider", ""),
                        "base_address": candidate.get("base_address", email),
                        "alias_index": candidate.get("alias_index"),
            ...
            if workspace_id:
                set_workspace_email_state(state_file, workspace_id, email, "failed", mode="login_existing", reason=str(error))
            ...
            while pending:
                ...
                for future in done:
                    if not future.cancelled():
                        future.result()
    logger.info(f"Existing account login complete: {len(results)} success, {len(candidates) - len(results)} failed/skipped")
    return results


def run_join_workspace(
    config: dict[str, Any], accounts: list[dict[str, Any]], cancel_event: threading.Event | None = None,
) -> list[dict[str, Any]]:
    ...
    proxy = str(config.get("proxy", {}).get("url", "")).strip()
    threads = _bounded_workers(_parallel_threads(config, "join_threads", 3), len(accounts))
    logger.info(f"Joining {len(accounts)} accounts to {len(workspace_ids)} workspace(s), {threads} worker(s)")
    state_file = workspace_state_path(config)
    ...
    def _join_one(account: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            for ws_id in workspace_ids:
                membership_map[ws_id] = _verify_workspace_membership(verify_session, access_token, ws_id)
            for result in results:
                result["request_ok"] = bool(result.get("ok"))
                active, detail = membership_map.get(str(result.get("workspace_id") or ""), (False, "workspace verification missing"))
        ...
        if all_ok:
            for ws_id in workspace_ids:
                set_workspace_email_state(
                    state_file, ws_id, email, "joined",
                    mode=str(account.get("source_type") or "account"), extra={"join_status": "ok"},
                )
            logger.info(f"[{email}] ✓ Joined {len(workspace_ids)} workspace(s)")
        else:
            errors = [
                r.get("error") or r.get("membership_detail") or "workspace membership not active"
                for r in results
                if (not r["ok"]) or not r.get("membership_active", request_ok)
            ]
            for ws_id in workspace_ids:
                set_workspace_email_state(
                    state_file, ws_id, email, "failed",
                    mode=str(account.get("source_type") or "account"),
                    reason=", ".join(str(e) for e in errors)[:300], extra={"join_status": "failed"},
                )
...
def run_re_login(
    ...
    ws_cfg = config.get("workspace", {})
    if not ws_cfg.get("re_login_enabled", False):
    ...
    mail_cfg = _mail_config_for_job(config, cancel_event)
    workspace_ids = _string_list(ws_cfg.get("ids", []))
    threads = _bounded_workers(_parallel_threads(config, "login_threads", 1), len(accounts))
    logger.info(f"Re-logging {len(accounts)} accounts for team-scoped tokens, {threads} worker(s)")
    ...
    def _login_one(account: dict[str, Any]) -> dict[str, Any]:
        ...
        if not email or not password:
            ...
            return account
        for key in (
            "team_access_token", "team_refresh_token", "team_id_token", "team_plan_type",
            "team_chatgpt_account_id", "team_account_user_role", "team_login_error",
        ):
        ...
        try:
            ...
            try:
                ...
                context = _fetch_account_context(
                    check_session, team_tokens["access_token"],
                    workspace_id=workspace_ids[0] if workspace_ids else "",
                )
                _apply_account_context(account, context, prefix="team_", overwrite_default=True)
                if not _is_workspace_plan(context.get("plan_type", "")):
                    raise RuntimeError(f"team token still resolved to personal scope (plan={context.get('plan_type') or 'unknown'})")
            finally:
                if check_session:
                    check_session.close()
            account["team_login_status"] = "ok"
            logger.info(f"[{email}] ✓ Team login successful (plan={account.get('team_plan_type', account.get('plan_type', '?'))})")
        except Exception as e:
...
def run_refresh_tokens(
    ...
    ws_cfg = config.get("workspace", {})
    workspace_ids = _string_list(ws_cfg.get("ids", []))
    workspace_plan = str(ws_cfg.get("export_plan_type") or DEFAULT_WORKSPACE_EXPORT_PLAN).strip() or DEFAULT_WORKSPACE_EXPORT_PLAN
    threads = _bounded_workers(_parallel_threads(config, "refresh_threads", 3), len(accounts))
    logger.info(f"Refreshing tokens and checking account info for {len(accounts)} accounts, {threads} worker(s)")
    state_file = workspace_state_path(config)
    ...
    def _refresh_one(account: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            session = _create_http_session(proxy)
            refresh_ok, refresh_detail = _refresh_oauth_tokens_for_account(session, account)
            if not refresh_ok:
            ...
            if at:
                ...
                context = _fetch_account_context(session, at, workspace_id=active_workspace_id)
                _apply_account_context(account, context)
                if active_workspace_id:
                    _apply_workspace_export_context(
                        account, active_workspace_id, str(account.get("plan_type") or workspace_plan)
                    )
                context_account_id = str(context.get("chatgpt_account_id") or "").strip()
                if workspace_ids and context_account_id in workspace_ids and _is_workspace_plan(context.get("plan_type", "")):
                    _mark_join_verified_by_check(account, context_account_id)
                    _apply_workspace_export_context(account, context_account_id, str(context.get("plan_type") or workspace_plan))
                ...
                if checked_workspace_id:
                    set_workspace_email_state(
                        state_file, checked_workspace_id, email, "checked",
                        mode=str(account.get("source_type") or "account"),
                        extra={
                            "plan_type": account.get("workspace_plan_type") or account.get("plan_type", ""),
                            "chatgpt_account_id": account.get("workspace_chatgpt_account_id") or account.get("chatgpt_account_id", ""),
                            "account_user_role": account.get("workspace_account_user_role") or account.get("account_user_role", ""),
                            "join_status": account.get("join_status", ""),
                            "join_verified_by": "accounts_check" if account.get("join_status") == "ok" else "",
                        },
                    )
            else:
            ...
            for ws_id in workspace_ids[:1]:
                set_workspace_email_state(
                    state_file, ws_id, email, "failed", mode=str(account.get("source_type") or "account"),
                    reason=detail, extra={"refresh_status": "failed"},
                )
...
def run_export(
    config: dict[str, Any], accounts: list[dict[str, Any]], output_file: Path | None = None,
) -> tuple[str, str]:
    team_setting = config.get("sub2api", {}).get("require_team_tokens", "auto")
    require_team = team_setting if isinstance(team_setting, bool) else bool(config.get("workspace", {}).get("re_login_enabled", False))
    if require_team:
        accounts = [
            ...
            if (
                ...
                and _is_workspace_plan(account.get("team_plan_type") or account.get("plan_type"))
            ) or (
                account.get("workspace_export_status") == "ok"
                and account.get("access_token")
                and account.get("workspace_chatgpt_account_id")
                and _is_workspace_plan(account.get("workspace_plan_type") or account.get("plan_type"))
                and account.get("refresh_status") != "failed"
                and not _has_auth_invalid_join_result(account)
            ) or (
                _has_verified_workspace_context(account) and not _has_auth_invalid_join_result(account)
            )
        ]
        if not accounts:
            raise RuntimeError("No verified team/workspace-scoped accounts available for export")
    ...
    json_str, actual_path = export_accounts_json(export_accounts, output_path, export_format)
    state_file = workspace_state_path(config)
    for account in export_accounts:
        ...
        if workspace_id and email and email != "?":
            set_workspace_email_state(
                state_file, workspace_id, email, "exported", mode=str(account.get("source_type") or "export"),
                extra={
                    "plan_type": account.get("workspace_plan_type") or account.get("plan_type", ""),
    ...
    logger.info(f"Exported {len(export_accounts)} accounts as {export_format} to {actual_path}")
    return json_str, actual_path


def run_full_pipeline(
    config: dict[str, Any], count: int | None = None, output_file: str | None = None,
    accounts_file: str | None = None, cancel_event: threading.Event | None = None,
) -> dict[str, Any]:
    config_dir = Path(config.get("_config_dir", "."))
    ...
    if not new_accounts:
        ...
        return {
            "registered": 0, "joined": 0, "refreshed": 0, "exported": 0,
            "accounts_file": str(af), "output_file": "",
        }
    _raise_if_cancelled(cancel_event)
    ...
    if re_login_enabled:
        _raise_if_cancelled(cancel_event)
        refreshed_accounts = run_re_login(config, joined_accounts, cancel_event=cancel_event)
    else:
        _raise_if_cancelled(cancel_event)
        refreshed_accounts = run_refresh_tokens(config, joined_accounts, cancel_event=cancel_event)
    save_accounts(af, refreshed_accounts)
    _raise_if_cancelled(cancel_event)
    all_accounts = load_accounts(af)
    if re_login_enabled:
        all_accounts = [account for account in all_accounts if account.get("team_login_status") == "ok"]
        if not all_accounts:
            return {
                "registered": len(new_accounts),
                "joined": sum(1 for a in refreshed_accounts if a.get("join_status") == "ok"),
                "refreshed": 0, "exported": 0, "accounts_file": str(af), "output_file": "",
            }
    try:
    ...
    refreshed = (
        ...
        if re_login_enabled
        else sum(1 for a in refreshed_accounts if _is_workspace_plan(a.get("plan_type")) and a.get("refresh_status") != "failed")
    )
