...
Supports:
  ...
  - gmail_password: email----app_password
"""
...
OUTLOOK_IN_USE_STALE_SECONDS = 300
...
def _plus_aliases(base_address: str, limit: int, alias_names: Any = None) -> list[tuple[int, str]]:
...
def _credential_unavailable(store: dict[str, dict[str, Any]], address: str, provider: str) -> bool:
    entry = store.get(_credential_state_key(address, provider))
    if isinstance(entry, dict) and str(entry.get("state") or "") in {"token_invalid", "failed"}:
        return True
...
def _state_entry(store: dict[str, dict[str, Any]], address: str, provider: str = "") -> dict[str, Any] | None:
...
def _save_state(state: dict[str, dict[str, Any]], state_file: Path = STATE_FILE) -> None:
    path = Path(state_file)
...
def _entry_available_for_workspace(entry: dict[str, Any] | None, *, workspace_scoped: bool) -> bool:
...
def _release_state(address: str, state_file: Path = STATE_FILE, provider: str = "") -> None:
    target = str(address or "").strip().lower()
    if not target:
        return
    with _mail_provider_state_lock:
...
def parse_gmail_password_credentials(text: str) -> list[dict[str, str]]:
    credentials: list[dict[str, str]] = []
    seen: set[str] = set()
    for raw_line in str(text or "").splitlines():
        line = str(raw_line or "").strip()
        if not line or "----" not in line:
            continue
        parts = [str(p).strip() for p in line.split("----", 1)]
        if len(parts) != 2:
            continue
        ...
        if "@" not in email or not app_password:
            continue
        key = email.lower()
        if key in seen:
            continue
        seen.add(key)
        credentials.append({"email": email, "app_password": app_password})
    return credentials
...
def _make_config(mail_config: dict) -> dict:
    ...
    return {
        ...
        "alias_limit_per_mailbox": _alias_limit(mail_config.get("alias_limit_per_mailbox") or 5, 5),
        "alias_custom_name_enabled": _bool_value(mail_config.get("alias_custom_name_enabled"), False),
        "alias_custom_names": mail_config.get("alias_custom_names") or "",
        "workspace_id": str(mail_config.get("workspace_id") or "").strip(),
        "workspace_state_file": str(mail_config.get("workspace_state_file") or "").strip(),
        "_cancel_event": mail_config.get("_cancel_event"),
    }
...
def _provider_entries(mail_config: dict) -> list[dict[str, Any]]:
    ...
    for item in providers:
        ...
        if provider_type not in {"outlook_token", "gmail_oauth", "gmail_password"}:
            continue
        refs[provider_type] = refs.get(provider_type, 0) + 1
        entries.append(dict(item, provider_ref=f"{provider_type}#{refs[provider_type]}"))
    return entries
...
def configured_mailboxes(mail_config: dict) -> list[dict[str, Any]]:
    ...
    for entry in _provider_entries(mail_config):
        ...
        if provider_type == "outlook_token":
            pool = parse_outlook_credentials(str(entry.get("mailboxes") or entry.get("pool") or ""))
            alias_enabled = _bool_value(entry.get("alias_enabled"), _bool_value(conf.get("alias_enabled"), False))
            alias_limit = _alias_limit(entry.get("alias_limit_per_mailbox") or conf.get("alias_limit_per_mailbox") or 6, 6)
            alias_names = (
                entry.get("alias_custom_names") or conf.get("alias_custom_names")
                if _bool_value(entry.get("alias_custom_name_enabled"), _bool_value(conf.get("alias_custom_name_enabled"), False))
                else None
            )
            for credential in pool:
                base_address = _base_plus_address(credential["email"])
                for alias_index, alias_address in _plus_aliases(base_address, alias_limit if alias_enabled else 1, alias_names):
                    candidates.append({
        ...
        elif provider_type == "gmail_oauth":
            for credential in parse_gmail_credentials(str(entry.get("mailboxes") or entry.get("pool") or "")):
                candidates.append({
                    "provider": provider_type,
                    "provider_ref": provider_ref,
                    "label": label,
        ...
        elif provider_type == "gmail_password":
            alias_enabled = _bool_value(entry.get("alias_enabled"), _bool_value(conf.get("alias_enabled"), False))
            alias_limit = _alias_limit(entry.get("alias_limit_per_mailbox") or conf.get("alias_limit_per_mailbox") or 6, 6)
            alias_names = (
                entry.get("alias_custom_names") or conf.get("alias_custom_names")
            ...
            for credential in parse_gmail_password_credentials(str(entry.get("mailboxes") or entry.get("pool") or "")):
                base_address = _base_plus_address(credential["email"])
                for alias_index, alias_address in _plus_aliases(base_address, alias_limit if alias_enabled else 1, alias_names):
                    candidates.append({
                        "provider": provider_type,
...
def _extract_code(message: dict[str, Any]) -> str | None:
    ...
    match = re.search(
        r"(?:Verification code|code is|Mã xác minh|mã xác minh|mã là)[:\s]*(\d{6})",
        content,
        re.I,
    )
...
def _message_code_priority(message: dict[str, Any], hint: str = "") -> tuple[int, float]:
    ...
    is_login = any(
        marker in subject
        for marker in ("login code", "sign-in code", "sign in code", "mã đăng nhập")
    )
    is_verification = any(
        marker in subject
        for marker in ("verification code", "mã xác minh", "xác minh")
    )
    if hint == "login":
...
def _mail_wait_cancelled(conf: dict[str, Any]) -> bool:
    cancel_event = conf.get("_cancel_event")
    return bool(cancel_event and hasattr(cancel_event, "is_set") and cancel_event.is_set())


def _mail_wait_sleep(conf: dict[str, Any]) -> bool:
    delay = max(0.2, float(conf.get("wait_interval") or 2))
    cancel_event = conf.get("_cancel_event")
...
class GmailPasswordError(MailProviderTokenError):
    pass
...
class BaseMailProvider:
    ...
    def wait_for_code(self, mailbox: dict[str, Any]) -> str | None:
        ...
        while time.monotonic() < deadline:
            if _mail_wait_cancelled(self.conf):
                return None
    ...
    def _imap_messages(self, mailbox: dict[str, Any], access_token: str, imap_host: str, label: str, message_limit: int) -> list[dict[str, Any]]:
        login_address = (
            str(mailbox.get("login_address") or mailbox.get("base_address") or "").strip()
            or str(mailbox["address"])
        )
        auth_string = f"user={login_address}\x01auth=Bearer {access_token}\x01\x01"
    ...
    def _imap_password_messages(self, mailbox: dict[str, Any], app_password: str, imap_host: str, label: str, message_limit: int) -> list[dict[str, Any]]:
        login_address = (
            str(mailbox.get("login_address") or mailbox.get("base_address") or "").strip()
            or str(mailbox["address"])
        )
        imap = imaplib.IMAP4_SSL(imap_host)
        try:
            imap.login(login_address, app_password)
            status, _ = imap.select("INBOX", readonly=True)
            if status != "OK":
                raise RuntimeError(f"{label} IMAP select INBOX failed")
            status, data = imap.uid("search", None, "ALL")
...
class OutlookTokenProvider(BaseMailProvider):
    ...
    def __init__(self, entry: dict, conf: dict):
        ...
        self.pool = parse_outlook_credentials(str(entry.get("mailboxes") or entry.get("pool") or ""))
        ...
        self.alias_enabled = _bool_value(entry.get("alias_enabled"), _bool_value(conf.get("alias_enabled"), False))
        self.alias_limit_per_mailbox = _alias_limit(entry.get("alias_limit_per_mailbox") or conf.get("alias_limit_per_mailbox") or 6, 6)
        self.alias_custom_name_enabled = _bool_value(entry.get("alias_custom_name_enabled"), _bool_value(conf.get("alias_custom_name_enabled"), False))
        self.alias_custom_names = entry.get("alias_custom_names") or conf.get("alias_custom_names") or ""
        self.state_file = Path(conf.get("state_file") or STATE_FILE)
        self.workspace_id = str(conf.get("workspace_id") or "").strip()
        self.workspace_state_file = Path(conf.get("workspace_state_file") or DATA_DIR / "workspace_account_state.json")
    ...
    def _exchange_refresh_token(self, client_id: str, refresh_token: str, scope: str) -> str:
        ...
        if resp.status_code != 200:
            ...
            raise OutlookTokenError(f"OutlookToken refresh failed: HTTP {resp.status_code}, {detail}")
        access_token = str(data.get("access_token") or "").strip()
        if not access_token:
            raise OutlookTokenError("OutlookToken refresh response missing access_token")
        return access_token

    def _cached_access_token(self, mailbox: dict[str, Any], client_id: str, refresh_token: str, scope: str) -> str:
    ...
    def create_mailbox(self, username: str | None = None) -> dict[str, Any]:
        if not self.pool:
            raise RuntimeError("OutlookToken pool is empty. Import email----password----client_id----refresh_token lines.")
        with _mail_provider_state_lock:
            ...
            for item in self.pool:
                ...
                for alias_index, alias_address in aliases:
                    if not _entry_available_for_workspace(_state_entry(store, alias_address, self.name), workspace_scoped=bool(self.workspace_id)):
                        continue
                    if self.workspace_id and not workspace_email_available(self.workspace_state_file, self.workspace_id, alias_address):
                        continue
                    if self.workspace_id and not claim_workspace_email(
                        ...
                        alias_address,
                        mode="register",
                        extra={"provider": self.name, "base_address": base_address, "alias_index": alias_index},
                    ):
                        continue
            ...
            if selected is None:
                raise RuntimeError(
                    f"[{self.label}] OutlookToken pool exhausted ({len(self.pool)} total). "
                    "All emails/aliases used/failed. Import new emails, increase alias_limit_per_mailbox, or reset pool state."
                )
    ...
    def _read_graph(self, access_token: str) -> list[dict[str, Any]]:
        ...
        if resp.status_code != 200:
            ...
            raise RuntimeError(f"OutlookToken Graph failed: HTTP {resp.status_code}, {detail}")
    ...
    def _normalize_graph_item(self, mailbox: dict[str, Any], item: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "provider": self.name,
            "mailbox": mailbox["address"],
            "login_mailbox": str(mailbox.get("login_address") or mailbox.get("base_address") or mailbox.get("address") or "").strip(),
    ...
    def fetch_recent_messages(self, mailbox: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if not client_id or not refresh_token:
            raise RuntimeError("OutlookToken mailbox missing client_id or refresh_token")
        errors: list[str] = []
        if self.mode in {"graph", "auto"}:
            try:
                access_token = self._cached_access_token(mailbox, client_id, refresh_token, OUTLOOK_GRAPH_SCOPE)
        ...
        if self.mode in {"imap", "auto"}:
            try:
                access_token = self._cached_access_token(mailbox, client_id, refresh_token, OUTLOOK_IMAP_SCOPE)
                return super()._imap_messages(mailbox, access_token, self.imap_host, "OutlookToken", self.message_limit)
            except Exception as error:
                if self.mode == "imap":
                    raise
...
class GmailOAuthProvider(BaseMailProvider):
    ...
    def __init__(self, entry: dict, conf: dict):
        super().__init__(conf, str(entry.get("provider_ref") or ""))
        self.label = str(entry.get("label") or self.provider_ref)
        self.pool = parse_gmail_credentials(str(entry.get("mailboxes") or entry.get("pool") or ""))
        self.imap_host = str(entry.get("imap_host") or GMAIL_DEFAULT_IMAP_HOST).strip() or GMAIL_DEFAULT_IMAP_HOST
        self.message_limit = max(1, int(entry.get("message_limit") or 10))
        self.state_file = Path(conf.get("state_file") or STATE_FILE)
        self.workspace_id = str(conf.get("workspace_id") or "").strip()
        self.workspace_state_file = Path(conf.get("workspace_state_file") or DATA_DIR / "workspace_account_state.json")
    ...
    def _exchange_refresh_token(self, client_id: str, client_secret: str, refresh_token: str) -> str:
        ...
        if resp.status_code != 200:
            detail = data.get("error_description") or data.get("error") or resp.text[:300]
            raise GmailOAuthError(f"GmailOAuth refresh failed: HTTP {resp.status_code}, {detail}")
        access_token = str(data.get("access_token") or "").strip()
        if not access_token:
            raise GmailOAuthError("GmailOAuth refresh response missing access_token")
        return access_token

    def _cached_access_token(self, mailbox: dict[str, Any], client_id: str, client_secret: str, refresh_token: str) -> str:
        ...
        if isinstance(cached, tuple) and len(cached) == 2 and time.monotonic() < cached[1]:
            return str(cached[0])
        token = self._exchange_refresh_token(client_id, client_secret, refresh_token)
        cache["imap"] = (token, time.monotonic() + 600)
        return token

    def create_mailbox(self, username: str | None = None) -> dict[str, Any]:
        if not self.pool:
            raise RuntimeError("Gmail OAuth pool is empty. Import email----client_id----client_secret----refresh_token lines.")
        with _mail_provider_state_lock:
            store = _load_state(self.state_file)
            credential = next((
                item for item in self.pool
                if _entry_available_for_workspace(_state_entry(store, item["email"], self.name), workspace_scoped=bool(self.workspace_id))
                and (not self.workspace_id or workspace_email_available(self.workspace_state_file, self.workspace_id, item["email"]))
                and (not self.workspace_id or claim_workspace_email(self.workspace_state_file, self.workspace_id, item["email"], mode="register", extra={"provider": self.name}))
            ), None)
            if credential is None:
                raise RuntimeError(f"[{self.label}] Gmail OAuth pool exhausted ({len(self.pool)} total). All emails used/failed. Import new emails or reset pool state.")
    ...
    def fetch_recent_messages(self, mailbox: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if not client_id or not client_secret or not refresh_token:
            raise RuntimeError("GmailOAuth mailbox missing client_id, client_secret, or refresh_token")
        access_token = self._cached_access_token(mailbox, client_id, client_secret, refresh_token)
        return super()._imap_messages(mailbox, access_token, self.imap_host, "GmailOAuth", self.message_limit)
...
class GmailPasswordProvider(BaseMailProvider):
    name = "gmail_password"

    def __init__(self, entry: dict, conf: dict):
        super().__init__(conf, str(entry.get("provider_ref") or ""))
        self.label = str(entry.get("label") or self.provider_ref)
        self.pool = parse_gmail_password_credentials(str(entry.get("mailboxes") or entry.get("pool") or ""))
        self.imap_host = str(entry.get("imap_host") or GMAIL_DEFAULT_IMAP_HOST).strip() or GMAIL_DEFAULT_IMAP_HOST
        self.message_limit = max(1, int(entry.get("message_limit") or 10))
        self.alias_enabled = _bool_value(entry.get("alias_enabled"), _bool_value(conf.get("alias_enabled"), False))
        self.alias_limit_per_mailbox = _alias_limit(entry.get("alias_limit_per_mailbox") or conf.get("alias_limit_per_mailbox") or 6, 6)
    ...
    def create_mailbox(self, username: str | None = None) -> dict[str, Any]:
        if not self.pool:
            raise RuntimeError("Gmail App Password pool is empty. Import email----app_password lines.")
        with _mail_provider_state_lock:
            store = _load_state(self.state_file)
            selected: tuple[dict[str, str], int, str] | None = None
            for item in self.pool:
            ...
            if selected is None:
                raise RuntimeError(
                    f"[{self.label}] Gmail App Password pool exhausted ({len(self.pool)} total). "
                    "All emails/aliases used/failed. Import new emails, increase alias_limit_per_mailbox, or reset pool state."
                )
            credential, alias_index, alias_address = selected
            base_address = _base_plus_address(credential["email"])
    ...
    def fetch_recent_messages(self, mailbox: dict[str, Any]) -> list[dict[str, Any]]:
        app_password = str(mailbox.get("app_password") or "").strip().replace(" ", "")
        if not app_password:
            raise GmailPasswordError("Gmail App Password mailbox missing app_password")
        try:
            return super()._imap_password_messages(mailbox, app_password, self.imap_host, "Gmail App Password", self.message_limit)
        except imaplib.IMAP4.error as error:
            raise GmailPasswordError(f"Gmail App Password IMAP login failed: {error}") from error
...
def _provider_class(provider_type: str):
    return {
        ...
        GmailPasswordProvider.name: GmailPasswordProvider,
...
def _entry_has_address(entry: dict[str, Any], address: str) -> bool:
    ...
    if provider_type == OutlookTokenProvider.name:
        ...
        credentials = parse_outlook_credentials(str(entry.get("mailboxes") or entry.get("pool") or ""))
    elif provider_type == GmailOAuthProvider.name:
        credentials = parse_gmail_credentials(str(entry.get("mailboxes") or entry.get("pool") or ""))
    elif provider_type == GmailPasswordProvider.name:
        target = _base_plus_address(target)
        credentials = parse_gmail_password_credentials(str(entry.get("mailboxes") or entry.get("pool") or ""))
    else:
        credentials = []
    if provider_type in {OutlookTokenProvider.name, GmailPasswordProvider.name}:
        return any(_base_plus_address(str(item.get("email") or "")) == target for item in credentials)
    return any(str(item.get("email") or "").strip().lower() == target for item in credentials)


def _fill_mailbox_credentials(mailbox: dict[str, Any], entry: dict[str, Any]) -> dict[str, Any]:
    ...
    if credential:
        ...
        for key in ("client_id", "refresh_token", "client_secret", "password", "app_password"):
            if credential.get(key):
                mailbox[key] = credential[key]
        if provider_type == OutlookTokenProvider.name:
            mailbox.setdefault("base_address", _base_plus_address(credential_address))
            mailbox.setdefault("login_address", credential_address)
        if provider_type == GmailPasswordProvider.name:
            mailbox.setdefault("base_address", _base_plus_address(credential_address))
            mailbox.setdefault("login_address", _base_plus_address(credential_address))
    return mailbox
...
def create_mailbox(mail_config: dict, username: str | None = None) -> dict:
    ...
    if not provider_entries:
        raise RuntimeError("No supported mail provider found in mail.providers config")
    conf = _make_config(mail_config)
    last_error = ""
...
def wait_for_code(mail_config: dict, mailbox: dict) -> str | None:
    ...
    entry = next((item for item in provider_entries if item.get("provider_ref") == provider_ref), None)
    if entry is None:
        entry = next((item for item in provider_entries if item.get("type") == provider_name and _entry_has_address(item, address)), None)
    if entry is None:
        entry = next((item for item in provider_entries if _entry_has_address(item, address)), None)
    if entry is None and provider_entries:
        entry = provider_entries[0]
    if entry is None:
        raise RuntimeError(f"No mail provider found (ref={provider_ref}, address={address})")
...
def mark_mailbox_result(mailbox: dict, *, success: bool, error: Exception | str | None = None) -> None:
    ...
    if provider_name not in {OutlookTokenProvider.name, GmailOAuthProvider.name, GmailPasswordProvider.name}:
        return
    address = str(mailbox.get("address") or "").strip()
    if not address:
        return
    base_address = str(mailbox.get("base_address") or _base_plus_address(address)).strip().lower()
    extra: dict[str, Any] = {}
    if provider_name in {OutlookTokenProvider.name, GmailPasswordProvider.name} and base_address:
    ...
    if success:
        _set_state(address, "used", state_file=state_file, provider=provider_name, extra=extra)
        if workspace_id and workspace_state_file:
            set_workspace_email_state(
                workspace_state_file, workspace_id, address, "registered", mode="register",
                extra={"provider": provider_name, **extra},
            )
        return
    ...
    token_invalid = (
        ...
        or "Gmail App Password" in reason
        or "access_token" in reason
    )
    state_name = "token_invalid" if token_invalid else "failed"
    _set_state(address, state_name, reason[:300], state_file=state_file, provider=provider_name, extra=extra)
    if token_invalid and provider_name in {OutlookTokenProvider.name, GmailPasswordProvider.name} and base_address:
        _set_state(
    ...
    if workspace_id and workspace_state_file:
        set_workspace_email_state(
            workspace_state_file, workspace_id, address, state_name, mode="register",
            reason=reason[:300], extra={"provider": provider_name, **extra},
        )
