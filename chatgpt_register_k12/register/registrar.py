...
class PlatformRegistrar:
    ...
    def _cf_retry(self, url: str, make_request, label: str = "") -> Any:
        ...
        from chatgpt_register_k12.register.session import refresh_clearance_and_retry
        ...
        if is_cloudflare_challenge(resp2) or resp2 is None:
            raise RegistrationError(
                f"[{label}] Still blocked after clearance refresh. Try changing IP/proxy."
            )
        return resp2
    ...
    def _register_user(self, email: str, password: str, index: int) -> None:
        ...
        if resp.status_code != 200:
            ...
            raise RegistrationError(
                f"user_register HTTP {resp.status_code}, {_response_debug(resp)}"
            )
    ...
    def _validate_otp(self, code: str, index: int) -> str:
        ...
        if resp2 is None or resp2.status_code != 200:
            ...
            try:
                ...
                pass
            raise RegistrationError(
                f"validate_otp HTTP {getattr(resp2, 'status_code', '?')}, body={body}"
            )
    ...
    def _create_account(self, name: str, birthdate: str, index: int) -> None:
        ...
        if not self.platform_auth_code:
            raise RegistrationError(
                f"No auth code in create_account response. status={resp.status_code}, "
    ...
    def _exchange_tokens(self, index: int) -> dict:
        ...
        if resp.status_code != 200:
            raise RegistrationError(
                f"token exchange HTTP {resp.status_code}, {_response_debug(resp)}"
            )
    ...
    def register(self, index: int = 0) -> dict:
        ...
        try:
            ...
            logging.getLogger(__name__).info(f"[Task {index}] Got verification code")
            next_page = self._validate_otp(code, index)
            if not self.platform_auth_code:
                if next_page not in {"about_you", "create_account"}:
                    if self.passwordless_flow and self._finalize_authorize_for_code(email, index):
                        next_page = "oauth_callback"
                    else:
                        raise RegistrationError(
                            f"OTP validation did not return an OAuth code or account creation page "
                            f"(page={next_page or '?'})"
                        )
            if not self.platform_auth_code:
                try:
            ...
            if not self.platform_auth_code:
                ...
                raise RegistrationError("Account creation completed without an OAuth code")
