...
from chatgpt_register_k12.export.formats import count_exported_json, output_filename_from_config
...
class JobManager:
    ...
    def create_job(self, action: str, params: dict[str, Any]) -> Job:
        ...
        if action not in {
            "run", "existing-token", "register", "login-existing",
            "join-workspace", "refresh", "export", "preview",
        }:
        ...
        try:
            ...
            if job.action == "run":
                ...
                logged_accounts = run_login_existing(
                    config, accounts_file, count=count_value, cancel_event=job.cancel_event,
                )
                if not logged_accounts:
                    raise RuntimeError("No existing accounts logged in")
                job.stage = "join-workspace"
                self._raise_if_cancelled(job)
                joined_accounts = run_join_workspace(config, logged_accounts, cancel_event=job.cancel_event)
                save_accounts(accounts_file, joined_accounts)
                job.stage = "refresh"
                self._raise_if_cancelled(job)
                refreshed_accounts = run_refresh_tokens(config, joined_accounts, cancel_event=job.cancel_event)
                ...
                job.stage = "register"
                accounts = run_register(config, accounts_file, count=count_value, cancel_event=job.cancel_event)
                job.summary = {"registered": len(accounts)}
            elif job.action == "login-existing":
                job.stage = "login-existing"
                accounts = run_login_existing(config, accounts_file, count=count_value, cancel_event=job.cancel_event)
                job.summary = {"logged_in": len(accounts)}
            elif job.action == "join-workspace":
                job.stage = "join-workspace"
                accounts = run_join_workspace(config, load_accounts(accounts_file), cancel_event=job.cancel_event)
                ...
                job.stage = "refresh"
                accounts = run_refresh_tokens(config, load_accounts(accounts_file), cancel_event=job.cancel_event)
                save_accounts(accounts_file, accounts)
