...
async function openRunFolder(job) {
  ...
  try {
    ...
    logsEl.textContent += `[WARNING] Không thể mở thư mục kết quả: ${err.message}\n`;
  }
}
...
cancelJobEl?.addEventListener("click", async () => {
  ...
  try {
    ...
    logsEl.textContent += data.ok
      ? "[WARNING] Đã gửi yêu cầu dừng; tác vụ sẽ dừng sau khi yêu cầu mạng hiện tại kết thúc.\n"
      : "[WARNING] Không thể dừng tác vụ hiện tại.\n";
...
document.querySelector("#refreshJobs").addEventListener("click", async () => {
  try {
    renderStatus(await api("/api/jobs"));
  } catch (err) {
    renderStatus({ error: err.message });
  }
});
...
document.querySelector("#runTerminal").addEventListener("click", runTerminalCommand);
...
providerInputs.forEach((input) => input.addEventListener("change", syncProviderPanels));
