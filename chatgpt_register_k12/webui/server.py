...
class WebUIHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, fmt: str, *args) -> None:
        ...
        is_polling = request_line.startswith("GET /api/jobs") or request_line.startswith("GET /api/health")
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        if path == "/api/config/save":
            ...
            try:
                ...
                if isinstance(workspace_ids_raw, list):
                    workspace_ids = [str(item).strip() for item in workspace_ids_raw if str(item).strip()]
                else:
                    workspace_ids = [item.strip() for item in str(workspace_ids_raw or "").splitlines() if item.strip()]
                if workspace_ids:
                ...
                config.setdefault("proxy", {})["url"] = str(body.get("proxy_url") or "").strip()
                ...
                alias_limit = min(6, max(1, int(body.get("alias_limit_per_mailbox") or 5)))
                alias_custom_name_enabled = bool(body.get("alias_custom_name_enabled", False))
                alias_custom_names = str(body.get("alias_custom_names") or "").strip()
                providers = config.setdefault("mail", {}).setdefault("providers", [])
                outlook = next((item for item in providers if isinstance(item, dict) and item.get("type") == "outlook_token"), None)
                if outlook is None:
                    outlook = {"type": "outlook_token", "enable": True, "label": "Outlook Pool", "mode": "auto"}
                    providers.insert(0, outlook)
                gmail_password = next((item for item in providers if isinstance(item, dict) and item.get("type") == "gmail_password"), None)
                if gmail_password is None:
                    gmail_password = {
                        "type": "gmail_password", "enable": False,
                        "label": "Gmail App Password Pool", "imap_host": "imap.gmail.com", "message_limit": 10,
                    }
                ...
                gmail_password["imap_host"] = str(gmail_password.get("imap_host") or "imap.gmail.com")
                gmail_password["message_limit"] = int(gmail_password.get("message_limit") or 10)
                if gmail_mailboxes:
        ...
        if path == "/api/jobs":
            try:
                job = self.manager.create_job(str(body.get("action") or ""), body)
            except Exception as error:
                self._json({"error": str(error)}, HTTPStatus.BAD_REQUEST)
                return
        ...
        if path == "/api/terminal/run":
            result = run_terminal_command(
                str(body.get("command") or ""), self.base_dir, timeout=int(body.get("timeout") or 120)
            )
