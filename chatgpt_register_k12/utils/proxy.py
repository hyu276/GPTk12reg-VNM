...
class FlareSolverrClearanceProvider:
    ...
    def fetch_clearance(self, target_url: str, proxy_url: str = "") -> ClearanceBundle | None:
        fs_url = f"{self._flaresolverr_url}/v1"
        payload: dict[str, Any] = {"cmd": "request.get", "url": target_url, "maxTimeout": self._timeout_ms}
        if proxy_url:
            payload["proxy"] = {"url": proxy_url}

        try:
            response = requests.post(
                fs_url, json=payload, headers={"Content-Type": "application/json"},
        ...
        return ClearanceBundle(
            target_host=host, proxy_url=proxy_url, cookies=cookies,
            user_agent=user_agent, created_at=time.time(), expires_at=1800.0,
        )
...
def build_session_kwargs(proxy: str = "", impersonate: str = "chrome", verify: bool = True) -> dict[str, Any]:
...
def create_session(proxy: str = "", impersonate: str = "chrome", verify: bool = True) -> requests.Session:
    kwargs = build_session_kwargs(proxy=proxy, impersonate=impersonate, verify=verify)
    return requests.Session(**kwargs)


def apply_clearance_to_session(session: requests.Session, bundle: ClearanceBundle | None) -> None:
    if bundle is None:
        return
