...
BLOCKING_STATES = {"in_use", "registered", "logged_in", "joined", "checked", "exported", "skipped"}
_state_lock = Lock()
...
def workspace_state_path(config_or_dir: dict[str, Any] | str | Path) -> Path:
    if isinstance(config_or_dir, dict):
        configured = str(config_or_dir.get("workspace_state", {}).get("file") or "").strip()
        if configured:
...
def save_workspace_state(path: str | Path, state: dict[str, Any]) -> None:
    ...
    ordered = {
        "workspaces": {
            ws_id: {email: entries[email] for email in sorted(entries) if isinstance(entries, dict)}
            for ws_id, entries in sorted(workspaces.items()) if isinstance(entries, dict)
        }
    }
    state_path.write_text(json.dumps(ordered, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")


def get_workspace_email_state(path: str | Path, workspace_id: str, email: str) -> dict[str, Any] | None:
...
def workspace_email_available(path: str | Path, workspace_id: str, email: str, blocking_states: set[str] | None = None) -> bool:
...
def set_workspace_email_state(
    path: str | Path, workspace_id: str, email: str, state_name: str, *,
    mode: str = "", reason: str = "", extra: dict[str, Any] | None = None,
) -> None:
    workspace_id = str(workspace_id or "").strip()
    email_key = normalize_email(email)
    if not workspace_id or not email_key:
    ...
    with _state_lock:
        ...
        entry: dict[str, Any] = {"state": str(state_name or "").strip() or "unknown", "updated_at": now_iso()}
        if mode:
            entry["mode"] = str(mode)
        if reason:
...
def claim_workspace_email(path: str | Path, workspace_id: str, email: str, *, mode: str = "", extra: dict[str, Any] | None = None) -> bool:
    if not workspace_email_available(path, workspace_id, email):
        return False
    set_workspace_email_state(path, workspace_id, email, "in_use", mode=mode, extra=extra)
    return True
...
def list_workspace_entries(path: str | Path, workspace_id: str = "") -> list[dict[str, Any]]:
    ...
    if not isinstance(workspaces, dict):
        return []
    selected = {str(workspace_id): workspaces.get(str(workspace_id), {})} if workspace_id else workspaces
